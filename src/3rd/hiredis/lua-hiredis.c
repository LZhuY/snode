...
static int lconn_commandex(lua_State * L)
{
  redisContext * pContext = check_connection(L, 1);
  ...
  int msize = objlen+1;
  void* ptr = (void*) malloc(sizeof(char*)*msize);
  char** argv = (char**) ptr;
  size_t* argvlen = (size_t*) malloc(sizeof(size_t)*msize);

  size_t len;
  int i=0;
  const char* cmd = lua_tolstring(L,-2,&len);
  argv[i]=(char*)cmd;
  argvlen[i]=len;
  i++;
  ...
  redisReply * pReply = (redisReply *)redisCommandArgv(
      pContext, i, (const char**)argv, argvlen);
